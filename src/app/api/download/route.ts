 ...
 */
function getYtDlpCommand(url: string): string {
    ...
    return new Promise<NextResponse>((resolve) => {
      exec(command, (error, stdout, stderr) => {
        if (error) {
          ...
          return resolve(NextResponse.json({ error: "Failed to process video: " + stderr }, { status: 500 }));
        }
        ...
        try {
          ...
          resolve(NextResponse.json({ error: "Invalid response from video processor" }, { status: 500 }));
        }
      });
    });
  } catch (err) {
